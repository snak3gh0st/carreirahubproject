...
import { FormAssignmentStatus, InvoiceStatus } from "@prisma/client";

import { prisma } from "@/lib/db";
import { t, type Language } from "@/lib/i18n/hub";
import { summarizeHubJobSearchActivities } from "@/lib/hub/job-search-records";
import { JobSearchQuickAdd } from "@/app/hub/registros/JobSearchRecordModal";

type HubHomeViewProps = {
  customerId: string;
  lang: Language;
  identity?: {
    name?: string | null;
...
function firstNameFrom(nameOrEmail: string | null | undefined) {
...
function PortalLink({
  ...
  if (readOnly) {
    return (
      <span className={`${className} cursor-not-allowed`} aria-disabled="true">
        {children}
      </span>
    );
  }
...
export async function HubHomeView({
  customerId,
  lang,
  identity,
  readOnly = false,
  ...
  const [customer, invoices, placementTest, realtimeTest, formAssignments, enrollment, deal] = await Promise.all([
  ...
  const firstName =
    firstNameFrom(customer.name) ||
    firstNameFrom(identity?.name) ||
    firstNameFrom(customer.email) ||
    firstNameFrom(identity?.email);
  const englishLevel =
