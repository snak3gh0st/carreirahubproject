...
export default function LegacyAdminBIPage() {
  ...
  const dateRange = (searchParams.get("dateRange") || "last90") as DateRangeParam;
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery<AdminBIData>({
    queryKey: ["admin-bi", dateRange, from, to],
    queryFn: async () => {
      const res = await fetch(`/api/analytics/admin-bi?${preservedWindow}`);
  ...
  const {
    ...
    queryKey: ["executive-bi-admin-summary", dateRange, from, to],
    queryFn: async () => {
  ...
  const executiveSubtitle = executiveData
    ? `${executiveData.areas.operations.summary} This admin legacy page stays available for team, funnel, and student detail, but the KPI strip above is pinned to the canonical executive cockpit so every BI surface speaks the same numbers.`
    : "Use the CEO cockpit as the canonical executive BI surface. This legacy admin page remains available for deeper sales, student, and funnel inspection below.";
  const executiveStatus = executiveData
    ? executiveData.areas.operations.freshness.summary
    : executiveIsError
      ? "Canonical executive summary is temporarily unavailable."
      : "Loading canonical executive summary.";
  const dateRangeOptions =
    dateRange === "custom"
      ? [...BASE_DATE_RANGE_OPTIONS, { value: "custom", label: "Custom range" }]
      : BASE_DATE_RANGE_OPTIONS;

  const handleDateRangeChange = (nextDateRange: DateRangeParam) => {
    ...
    router.push(`/dashboard/bi/legacy?${params.toString()}`);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        <LegacyExecutiveSummary
          ...
          title="Operations and Delivery Detail"
          ...
          actions={
            <>
              ...
              <Link
                href={`/dashboard/financial?${preservedWindow}`}
                className="inline-flex items-center gap-2 rounded-full bg-white/12 px-3 py-1.5 text-sm font-medium text-white ring-1 ring-white/20 transition hover:bg-white/18"
              >
                Finance deep dive
              </Link>
              {dateRange === "custom" && (from || to) ? (
                <span className="rounded-full bg-amber-400/18 px-3 py-1.5 text-sm font-medium text-amber-50 ring-1 ring-amber-200/20">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Operations Detail Views</h1>
            <p className="text-sm text-gray-500 mt-0.5">
              Secondary support surface for commercial, operational, and student diagnostics.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="relative">
              <select
                value={dateRange}
                onChange={(e) => handleDateRangeChange(e.target.value as DateRangeParam)}
