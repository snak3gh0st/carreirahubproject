...
export const searchCustomers = defineAiTool({
  ...
  allowedRoles: [UserRole.ADMIN, UserRole.COMMERCIAL, UserRole.HEAD_COMERCIAL, UserRole.FINANCE],
  inputSchema: z.object({
    query: z.string().min(2),
    limit: z.number().int().min(1).max(50).default(20),
  }),
  async handler({ query, limit }, ctx) {
    requireRole(ctx.user.role, [UserRole.ADMIN, UserRole.COMMERCIAL, UserRole.HEAD_COMERCIAL, UserRole.FINANCE]);
    try {
