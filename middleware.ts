...
function getAuditBaseUrl(request: NextRequest) {
  const host = request.headers.get("x-forwarded-host") || request.headers.get("host");
  const protocol = request.headers.get("x-forwarded-proto") || "https";
  if (host) return `${protocol}://${host}`;
  return process.env.NEXT_PUBLIC_APP_URL || process.env.NEXTAUTH_URL || request.url;
}
...
function queueAccessAudit(
  ...
  const url = new URL("/api/internal/access-audit", getAuditBaseUrl(request));
  const payload = {
