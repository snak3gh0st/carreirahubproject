...
export default async function HubStatusPage() {
  ...
  const steps: Step[] = [
    {
      ...
      detail: contractSigned
        ? `Signed${contractDate ? ` ${new Date(contractDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}` : ""}`
        : "Pending signature",
    ...
    },
    {
      ...
      detail: anyPaid ? `${paidCount}/${totalInvoices} invoices paid` : "Awaiting first payment",
  ...
  if (onboardingDone !== null) {
    steps.push({
      ...
      detail: onboardingDone ? "All forms completed" : `${completedForms}/${totalForms} forms`,
  ...
  steps.push({
    ...
    detail: testDone ? `${placementTest!.displayLevel} (${placementTest!.cefrLevel})` : "Not taken yet",
  ...
  steps.push({
    ...
    detail: allPriorDone ? "Your process is active" : "Complete previous steps",
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">Track the status of your process with Carreira U.S.A.</p>
      </div>
