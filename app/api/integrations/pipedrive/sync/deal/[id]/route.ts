...
/**
 * POST /api/integrations/pipedrive/sync/deal/:id
 * Manually trigger Deal → Pipedrive sync
 */
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Deal sync to Pipedrive queued successfully",
      dealId,
    });
  } catch (error) {
    console.error("[API] Error syncing deal to Pipedrive:", error);

    return NextResponse.json(
      {
        error: "Failed to sync deal to Pipedrive",
